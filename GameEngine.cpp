...
BOOL GameEngine::CheckSpriteCollision(Sprite* pTestSprite)
{
    ...
    for (siSprite = m_vSprites.begin(); siSprite != m_vSprites.end(); siSprite++)
    {
        ...
        // Test the collision
        if (pTestSprite->CircularTestCollision(*siSprite)) {
            // Collision detected
...
void GameEngine::UpdateSprites()
{
    ...
    for (siSprite = m_vSprites.begin(); siSprite != m_vSprites.end(); /* siSprite++ */)
    {
        ...
        if ((*siSprite)->GetConnected()) {
            setBallPosition((*siSprite));
            OpponentAttack((*siSprite));
        }
        siSprite++;
    }
}
...
void GameEngine::ResetConnected()
{
    ...
    {
        if ((*siSprite)->GetConnected()) {
            (*siSprite)->SetAvailCount(15);
        }
        (*siSprite)->SetConnected(FALSE);
    }
