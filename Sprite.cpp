...
Sprite::Sprite(Bitmap* pBitmap, RECT& rcBounds, BOUNDSACTION baBoundsAction)
{
    ...
    m_bDying = FALSE;
    m_bOneCycle = FALSE;
    m_pCenter.x = (m_rcPosition.right + m_rcPosition.left) / 2;
    m_pCenter.y = (m_rcPosition.top + m_rcPosition.bottom) / 2;
    m_fRadius = (m_rcPosition.right - m_rcPosition.left) / 2;
    m_bConnected = false;
}

Sprite::Sprite(Bitmap* pBitmap, RECT& rcBounds, int x, int y, int id, BOUNDSACTION baBoundsAction)
{
    // Calculate a random position
    int iXPos = rand() % (rcBounds.right - rcBounds.left);
    int iYPos = rand() % (rcBounds.bottom - rcBounds.top);
    ...
    m_iNumFrames = 1;
    m_iCurFrame = m_iFrameDelay = m_iFrameTrigger = 0;
    SetRect(&m_rcPosition, x, y, x + pBitmap->GetWidth(),
        y + pBitmap->GetHeight());
    CalcCollisionRect();
    m_ptVelocity.x = m_ptVelocity.y = 0;
    m_iZOrder = 0;
...
SPRITEACTION Sprite::Update()
{
    ...
    else if (m_baBoundsAction == BA_BOUNCE)
    {
        ...
        {
            if (ptNewPosition.y > 269 && ptNewPosition.y < 373) {
                if (ptNewPosition.x < m_rcBounds.left - 21) {
                    ptNewPosition.x = m_rcBounds.left - 21;
        ...
        {
            if (ptNewPosition.y > 269 && ptNewPosition.y < 373) {
                if ((ptNewPosition.x + ptSpriteSize.x) > m_rcBounds.right + 21) {
                    ptNewPosition.x = m_rcBounds.right - ptSpriteSize.x + 21;
                    ptNewVelocity.x = -ptNewVelocity.x;
                    bBounce = TRUE;
                }
            }
